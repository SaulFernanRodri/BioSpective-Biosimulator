...
public class SphereMolecule extends SpherePortrayal3D implements iMolecule, Serializable {
	...
	private boolean isNeccesaryChangeColor = false;
	...
	public SphereMolecule(int id, String name, double weigth, double radius, String color, double drExterior,
	...
	public boolean firstPrepare(SimState state) {
		...
		// Comprobar eventos (kill y/o
		// transform). 0: Exito, 2: No se pudo llevar a cabo
		if (cs.hasEvents && !isInDoor()) {
			int resolved = 2;
	...
	public int secondPrepare(SimState state, int rootID, int extent) {
		...
		if (!this.inDoor && timeToWait == 0 && !this.toStop) {
			...
			// System.out.println("UNSTOPPABLE: " + isUnstoppable + ", " +
			// name);
			// System.out.println("TORET: " + toRet + ", " + name);
		} else {
			this.acceptablePosition = true;
		}
	...
	public void move(SimState state) {
		...
		if (!this.toStop) {
			if (desiredLocation != null && this.acceptablePosition && timeToWait == 0) {
				...
				// If molecule finish a reaction, increment number in the map
				if (timeToWait == 0) {
					putReaction(false);
					// setUnstoppable(true);
				}
	...
	public TransformGroup getModel(Object obj, TransformGroup j3dModel) {
		// First time or if the color has to change
		if (j3dModel == null) {
			// Set color to the ball
			setAppearance(j3dModel,
					appearanceForColors(color, //
			...
			// Set scale
			setScale(j3dModel, (radius * 2));
		}

		if (changeColor && isNeccesaryChangeColor) {
			setAppearance(j3dModel,
					appearanceForColors(Functions.getContrastColor(color), //
							null, // emissive color (black)
							Functions.getContrastColor(color), // diffuse color
							null, // specular color (white)
							1.0f, // no shininess
							1.0f)); // full opacity
		} else if (!changeColor && isNeccesaryChangeColor) {
			setAppearance(j3dModel,
					appearanceForColors(color, //
							null, // emissive color (black)
							color, // diffuse color
	...
	// public Double3D calculateDesiredWithSpeed(Vector3D newSpeed) {
	// // Generate movement
	// return new Double3D(this.currentLocation.x + newSpeed.x,
	// this.currentLocation.y + newSpeed.y, this.currentLocation.z
	// + newSpeed.z);
	// }
	//
	// public Double3D calculateDesiredWithSpeed(Double3D molLocation,
	// Vector3D newSpeed) {
	// // Generate movement
	// return new Double3D(molLocation.x + newSpeed.x, molLocation.y
	// + newSpeed.y, molLocation.z + newSpeed.z);
	// }
